...
CPython manages object lifetimes through reference counting, supplemented by a cycle-detecting garbage collector. Each object maintains a count of references to it, and when this count reaches zero, the object is deallocated.
...
CPython uses open addressing to handle collisions. When a collision occurs, it probes for the next available slot using a pseudorandom sequence based on the hash value. This approach is known as "open addressing with random probing."
...
class Polynomial:
    """A polynomial class with various dunder methods."""
    
    def __init__(self, *coefficients):
        """Initialize with coefficients (highest power first)."""
    ...
    def __add__(self, other):
        ...
        result_coeffs = [0] * max(len(self.coefficients), len(other.coefficients))
        
        for i, coef in enumerate(self.coefficients):
            result_coeffs[i + (len(result_coeffs) - len(self.coefficients))] += coef
            
        for i, coef in enumerate(other.coefficients):
            result_coeffs[i + (len(result_coeffs) - len(other.coefficients))] += coef
    ...
    def __sub__(self, other):
        ...
        # Negate coefficients of other and add
    ...
    def __rsub__(self, other):
        ...
        # Negate coefficients of self and add to other
        neg_self = Polynomial(*[-c for c in self.coefficients])
        return other.__add__(neg_self)
    ...
    def __mul__(self, other):
        ...
        for i, c1 in enumerate(self.coefficients):
            for j, c2 in enumerate(other.coefficients):
                power = (len(self.coefficients) - 1 - i) + (len(other.coefficients) - 1 - j)
                result_coeffs[result_degree - power] += c1 * c2
    ...
    def __len__(self):
        """Return the number of terms (non-zero coefficients)."""
        return sum(1 for c in self.coefficients if c != 0)
    
    def __getitem__(self, power):
        """Get the coefficient of x^power."""
        degree = len(self.coefficients) - 1
        if power < 0 or power > degree:
            raise IndexError("Power out of range")
        return self.coefficients[degree - power]
    
    def __setitem__(self, power, value):
        """Set the coefficient of x^power."""
        degree = len(self.coefficients) - 1
        if power < 0 or power > degree:
            raise IndexError("Power out of range")
        self.coefficients[degree - power] = value
    ...
    def __eq__(self, other):
        ...
        self_coeffs = self.coefficients.copy()
        while len(self_coeffs) > 1 and self_coeffs[0] == 0:
            self_coeffs.pop(0)
            
        other_coeffs = other.coefficients.copy()
        while len(other_coeffs) > 1 and other_coeffs[0] == 0:
            other_coeffs.pop(0)
    ...
    def derivative(self):
        ...
        for i, coef in enumerate(self.coefficients[:-1]):
            power = degree - i
            result_coeffs.append(coef * power)
...
print(f"p1 - p2 = {p1 - p2}")
print(f"p1 * p2 = {p1 * p2}")
print(f"p1(2) = {p1(2)}")  # Evaluate at x=2
print(f"Coefficient of x^1 in p1: {p1[1]}")
...
### Descriptors: `__get__`, `__set__`, `__delete__`, and Use Cases

Descriptors are a powerful way to create reusable property behavior in Python. They're the mechanism behind properties, class methods, and static methods.

```python
class LazyProperty:
    """A descriptor that computes a value only when needed and caches it."""
    ...
    def __get__(self, instance, owner):
        ...
        # Compute the value if it doesn't exist
...
class Circle:
    """A circle with lazily computed properties."""
    ...
    @LazyProperty
    def area(self):
        """Compute the area (expensive operation)."""
    ...
    @LazyProperty
    def circumference(self):
        """Compute the circumference (expensive operation)."""
...
print(f"Area: {circle.area}")  # Computing area..., 78.54...

# Second access uses the cached value
print(f"Area again: {circle.area}")  # 78.54... (no computation message)

# First access to another property
print(f"Circumference: {circle.circumference}")  # Computing circumference..., 31.42...

# Unit conversion descriptor
class Unit:
    """A descriptor for converting between units."""
    
    def __init__(self, unit_from, unit_to, factor):
        """Initialize with conversion units and factor."""
    ...
    def __get__(self, instance, owner):
        ...
        internal_value = getattr(instance, self.internal_name, 0)
        display_value = internal_value / self.factor
        return display_value
    
    def __set__(self, instance, value):
        """Called when the attribute is set."""
        # Convert the value to the internal unit
        internal_value = value * self.factor
        setattr(instance, self.internal_name, internal_value)
...
class Distance:
    """A class for distances with unit conversion."""
    
    # 1 kilometer = 1000 meters
    kilometers = Unit("km", "m", 1000)
    
    # 1 mile = 1609.34 meters
    miles = Unit("mi", "m", 1609.34)
    
    # The base unit is meters
    meters = Unit("m", "m", 1)
    
    def __init__(self, **kwargs):
        """Initialize with distances in various units."""
        for unit, value in kwargs.items():
            setattr(self, unit, value)

# Using the unit conversion
distance = Distance(kilometers=5)
print(f"{distance.kilometers} km")   # 5.0 km
print(f"{distance.meters} m")        # 5000.0 m
print(f"{distance.miles} mi")        # 3.1069 mi

# Change the distance in a different unit
distance.miles = 10
print(f"{distance.kilometers} km")   # 16.0934 km
print(f"{distance.meters} m")        # 16093.4 m
print(f"{distance.miles} mi")        # 10.0 mi

# Database field descriptor
class Field:
    """A descriptor for database fields."""
    
    def __init__(self, field_type, required=False, default=None):
        """Initialize with field type, required flag, and default value."""
    ...
    def __set__(self, instance, value):
        ...
        if value is not None and not isinstance(value, self.field_type):
            raise TypeError(f"{self.name} must be a {self.field_type.__name__}")
...
print(user.to_dict())  # {'id': 1, 'name': 'Alice', 'email': 'alice@example.com', 'age': 30, 'active': True}

try:
    # Validate will fail if required fields are missing
    empty_user = User()
    empty_user.validate()
except ValueError as e:
    print(f"Error: {e}")
...
class CachedProperty:
    """A property that caches its value but can be invalidated."""
...
class Rectangle:
    """A rectangle with cached properties that depend on dimensions."""
    ...
    @CachedProperty
    def area(self):
        """Calculate the area (cached)."""
        print("Computing area...")
        return self.width * self.height
    
    @CachedProperty
    def perimeter(self):
        """Calculate the perimeter (cached)."""
        print("Computing perimeter...")
        return 2 * (self.width + self.height)
    
    # Method to update dimensions and invalidate cached properties
    def resize(self, width, height):
        """Resize the rectangle and invalidate cached properties."""
        self.width = width
        self.height = height
        
        # Invalidate cached properties
        self.__class__.area.invalidate(self)
        self.__class__.perimeter.invalidate(self)

# Using the cached properties
rect = Rectangle(3, 4)
print(f"Area: {rect.area}")       # Computing area..., 12
print(f"Area again: {rect.area}")  # 12 (cached)
print(f"Perimeter: {rect.perimeter}")  # Computing perimeter..., 14

# Resize and check that caches are invalidated
rect.resize(5, 6)
print(f"New area: {rect.area}")  # Computing area..., 30
...
Python provides tools for packaging and distributing your code as modules or applications, making it easy to share your work with others.
...
def setup_py_example():
    """Provide an example setup.py file."""
    print("Example setup.py file:")
    print("""
...
""")
    print("\nThis setup.py file defines:")
    print("- Basic metadata (name, version, author, description)")
    print("- Package discovery with find_packages()")
    print("- Classifiers for indexing on PyPI")
    print("- Required Python version")
    print("- Dependencies (required and optional)")
    print("- Command-line entry points")
    print("- Package data files to include")

# Package structure example
def package_structure():
    """Describe a typical Python package structure."""
...
""")
    print("\nKey files:")
    print("- LICENSE: Your project's license (e.g., MIT, Apache)")
    print("- README.md: Project description, installation and usage instructions")
    print("- pyproject.toml: PEP 517/518 build system specification")
    print("- setup.py: Installation script")
    print("- setup.cfg: Configuration for setup.py")
    print("- MANIFEST.in: Additional files to include in distributions")
    print("- requirements.txt: Dependencies for development or installation")
    print("- mypackage/__init__.py: Makes the directory a package")
    print("- tests/: Unit tests for your package")
    print("- docs/: Documentation files")

# pyproject.toml example
def pyproject_toml_example():
    """Provide an example pyproject.toml file."""
...
testpaths = ["tests"]
""")
    print("\nThis pyproject.toml file defines:")
    print("- Build system requirements (PEP 517/518)")
    print("- Configuration for Black code formatter")
    print("- Configuration for isort import sorter")
    print("- Configuration for pytest")

# Building and distributing
def building_and_distributing():
    """Describe the process of building and distributing packages."""
    print("\nBuilding and distributing your package:")
    
    print("\n1. Create source distribution (sdist):")
    print("   python setup.py sdist")
    print("   or: python -m build --sdist")
    
    print("\n2. Create wheel distribution (bdist_wheel):")
    print("   python setup.py bdist_wheel")
    print("   or: python -m build --wheel")
    ...
    print("\nFor automated builds and uploads, consider using GitHub Actions or other CI/CD tools.")

# Entry points and console scripts
def entry_points_example():
    """Explain entry points and console scripts."""
    print("\nEntry points and console scripts:")
    
    print("\nIn setup.py, you can define entry points:")
    print("""
entry_points={
    'console_scripts': [
        'mycommand=mypackage.cli:main',
...
""")
    ...
    print("\nExample CLI module (mypackage/cli.py):")
    print("""
def main():
    ...
    if args.version:
        ...
        print(f"mypackage version {__version__}")
        return
...
""")
    ...
    print("\nAfter installation, users can run your script from the command line:")
    print("  $ mycommand --version")
    print("  $ mycommand input.txt")

# Package versioning
def package_versioning():
    """Explain package versioning strategies."""
    print("\nPackage versioning strategies:")
    
    print("\n1. Single-sourced versioning:")
    print("   Define version in one place and import it elsewhere")
...
""")
    ...
    print("\n2. Using a separate version file:")
    print("   Version defined in a separate file (e.g., _version.py)")
    
    print("\nExample in mypackage/_version.py:")
    print("""
__version__ = '0.1.0'
""")
...
""")
    ...
    print("\n3. Using setuptools_scm:")
    print("   Automatically derive version from git tags")
...
def package_documentation():
    """Explain documentation approaches for Python packages."""
    print("\nDocumenting your package:")
    
    print("\n1. Docstrings in your code:")
    print("   Use Google, NumPy, or reStructuredText format")
    
    print("\nExample of Google-style docstring:")
    print("""
def process_data(data, config=None):
    \"\"\"Process the input data using the provided configuration.
...
""")
    ...
    print("\n2. Generating documentation with Sphinx:")
    print("   - Install with 'pip install sphinx sphinx-rtd-theme'")
    print("   - Initialize with 'sphinx-quickstart'")
    print("   - Use sphinx-apidoc to auto-generate API docs from docstrings")
    print("   - Build with 'make html'")
    
    print("\n3. Read the Docs integration:")
    print("   - Create .readthedocs.yml in your repository")
    print("   - Link your GitHub repository to Read the Docs")
...
def package_testing():
    """Explain testing approaches for Python packages."""
    print("\nTesting your package:")
...
def packaging_best_practices():
    """Provide best practices for Python packaging."""
    print("\nPackaging best practices:")
    
    print("\n1. Package organization:")
    print("   - Use a clear, logical directory structure")
    print("   - Separate concerns into modules and subpackages")
    print("   - Use __init__.py to control public API")
    
    print("\n2. Dependencies:")
    print("   - Specify minimum versions but avoid overly restrictive upper bounds")
    print("   - Use environment markers for platform-specific dependencies")
    print("   - Separate optional dependencies with extras_require")
    
    print("\n3. Building and distributing:")
    print("   - Use pyproject.toml for build system specification")
    print("   - Always include both sdist and wheel distributions")
    print("   - Use twine for secure uploads to PyPI")
    
    print("\n4. Versioning:")
    print("   - Follow semantic versioning (MAJOR.MINOR.PATCH)")
    print("   - Consider using setuptools_scm for Git-based versioning")
    
    print("\n5. Documentation:")
    print("   - Write comprehensive docstrings")
    print("   - Include a detailed README.md")
    print("   - Consider generating API docs with Sphinx")
    
    print("\n6. Testing:")
    print("   - Write unit tests for all functionality")
    print("   - Set up CI/CD for automated testing")
    print("   - Track code coverage")
    
    print("\n7. Metadata:")
    print("   - Include proper classifiers")
    print("   - Specify license correctly")
    print("   - Include a clear description and long_description")

# Example of modern Python packaging with pyproject.toml
def modern_packaging_example():
    """Show a complete modern packaging example using pyproject.toml."""
    print("\nModern packaging example (PEP 621):")
...
""")
    ...
    print("- Uses PEP 621 metadata specification")
    print("- Centralizes configuration in pyproject.toml")
    print("- Standardizes project metadata")
    print("- Simplifies dependency specification")
    print("- Works with the latest Python packaging tools")

# Run the packaging examples
# setup_py_example()
# package_structure()
# pyproject_toml_example()
# building_and_distributing()
# entry_points_example()
# package_versioning()
# package_documentation()
# package_testing()
# packaging_best_practices()
# modern_packaging_example()
```
