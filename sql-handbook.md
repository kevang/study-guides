...
* [2. Dimensional Modeling Concepts](#2-dimensional-modeling-concepts)
* [3. Star and Snowflake Schemas](#3-star-and-snowflake-schemas)
* [4. Time and Date Dimensions](#4-time-and-date-dimensions)
* [5. Data Quality and Governance](#5-data-quality-and-governance)
* [6. Data Security and Access Control](#6-data-security-and-access-control)
* [7. Data Warehouse and Lake Architecture](#7-data-warehouse-and-lake-architecture)
...
Database normalization is the process of organizing data to minimize redundancy and improve data integrity. While normalization is primarily associated with OLTP (Online Transaction Processing) systems, understanding normalization forms is essential for analytics engineers who must often work with both normalized source systems and denormalized analytical schemas.

#### Normal Forms Overview

Normal forms represent stages of normalization, each addressing specific data redundancy issues:

```sql
-- First Normal Form (1NF): Eliminate repeating groups
-- A table is in 1NF when:
-- 1. Each column contains atomic values (no lists or arrays)
-- 2. Each column contains values of a single type
-- 3. Each row is unique
-- 4. Order does not affect data integrity

-- VIOLATION: Non-atomic values in a column
CREATE TABLE violation_1nf (
    order_id INTEGER,
    customer_name VARCHAR(100),
    products_ordered VARCHAR(500)  -- Contains multiple products
);

-- CORRECT: 1NF-compliant design
...
-- 1. It is in 1NF
-- 2. No non-key column is dependent on only part of a composite key
...
-- CORRECT: 2NF-compliant design (separate entities)
...
CREATE TABLE violation_3nf (
    ...
    store_region VARCHAR(50)  -- Transitive dependency
);

-- CORRECT: 3NF-compliant design
...
-- VIOLATION: BCNF violation
CREATE TABLE violation_bcnf (
...
CREATE TABLE course_instructor (
    course_id VARCHAR(10) REFERENCES courses_bcnf(course_id),
    instructor_id INTEGER REFERENCES instructors_bcnf(instructor_id),
    PRIMARY KEY (course_id, instructor_id)
);
```

#### Normalization in Analytics Contexts

While normalized schemas are ideal for transaction systems, analytical databases often intentionally denormalize for performance:

```sql
-- Normalized OLTP-style schema (for source systems)
...
CREATE TABLE normalized_order_items (
    order_item_id INTEGER PRIMARY KEY,
    order_id INTEGER REFERENCES normalized_orders(order_id),
    product_id INTEGER REFERENCES products(product_id),
    quantity INTEGER,
    unit_price NUMERIC(12,2)
);

-- Denormalized analytical schema (for reporting)
...
Understanding when to apply each approach is critical for analytics engineering:

```sql
-- Normalization is preferred when:
-- 1. Data needs frequent updates
-- 2. Storage costs are a concern
-- 3. Data integrity is paramount
-- 4. You need to track historical changes (SCD Type 2)
...
-- 2. Queries are primarily analytical (aggregations, reporting)
-- 3. Data is largely static (historical data)
-- 4. Simplifying queries for business users
...
#### Hybrid Approaches for Analytics

Many analytical databases use a hybrid approach, normalizing at the foundation while denormalizing for performance:

```sql
-- Layered approach in a data warehouse:

-- 1. Landing layer: Raw data (often denormalized)
...
-- 2. Staging layer: Cleaned and validated
...
-- 3. Integration layer: Fully normalized (3NF/BCNF)
...
CREATE TABLE staging_transactions (
    ...
    transaction_date TIMESTAMP,
    customer_id INTEGER REFERENCES staging_customers(customer_id)
);

CREATE TABLE staging_line_items (
    line_item_id SERIAL PRIMARY KEY,
    transaction_id VARCHAR(50) REFERENCES staging_transactions(transaction_id),
    product_id INTEGER REFERENCES staging_products(product_id),
    quantity INTEGER,
    amount NUMERIC(12,2)
);

-- 4. Presentation layer: Fully denormalized for reporting (star schema)
...
#### Practical Normalization Patterns
...
-- Analyzing enrollments with the normalized structure
SELECT 
    c.course_name,
...
```sql
-- Attribute splitting for flexibility
CREATE TABLE normalized_address (
    customer_id INTEGER PRIMARY KEY,
    customer_name VARCHAR(100),
...
-- Query by city or region efficiently
SELECT 
    city,
    COUNT(*) as customer_count
FROM 
...
#### Normalization Anti-Patterns in Analytics

Avoid these common mistakes when designing analytical schemas:

```sql
-- ANTI-PATTERN 1: Using natural keys as foreign keys in fact tables
...
-- BEST PRACTICE: Use surrogate keys (integer IDs)
...
-- ANTI-PATTERN 2: Storing derived/calculated values
CREATE TABLE fact_sales_calculated (
    sale_id INTEGER PRIMARY KEY,
    quantity INTEGER,
    unit_price NUMERIC(12,2),
...
-- BEST PRACTICE: Store only atomic values, calculate in queries
...
-- Calculate derived values in queries
SELECT 
    sale_id,
    quantity,
    unit_price,
...
-- ANTI-PATTERN 3: Using NULLs inappropriately
CREATE TABLE customer_analysis_bad (
    customer_id INTEGER PRIMARY KEY,
    customer_name VARCHAR(100),
    total_purchases NUMERIC(12,2),
    purchase_count INTEGER,
    avg_purchase_amount NUMERIC(12,2)
);

-- BEST PRACTICE: Use COALESCE or handle NULLs in queries
SELECT 
    customer_id,
    customer_name,
    ...
    CASE 
        WHEN purchase_count > 0 THEN avg_purchase_amount
        ELSE 0
...
Data quality and governance are foundational for reliable analytics. Poor data quality leads to incorrect insights, and without proper governance, organizations risk compliance violations and inconsistent reporting.

#### Data Quality Dimensions

Data quality encompasses several key dimensions that determine the usefulness of data for analytics:
...
-- Detect potential data accuracy issues
-- Example: Invalid email formats
SELECT 
    customer_id,
    customer_name,
    email
...
-- Example: Invalid dates (future dates in birth_date)
SELECT 
    customer_id,
    customer_name,
    birth_date
...
-- Example: Outlier detection for numeric values
WITH stats AS (
    SELECT 
...
SELECT 
    transaction_id,
    sales_amount,
    (sales_amount - avg_sales) / NULLIF(stddev_sales, 0) as z_score
FROM 
    sales_transactions, stats
WHERE 
...
SELECT 
    ...
    dw.customer_name as warehouse_name,
    CASE 
        WHEN c.customer_name = dw.customer_name THEN 'Match'
        ELSE 'Mismatch'
    END as comparison_result
FROM 
    source.customers c
JOIN 
    datawarehouse.dim_customer dw ON c.customer_id = dw.customer_id
WHERE 
    c.customer_name <> dw.customer_name;

-- Detect inconsistent categorization
SELECT 
    product_id,
...
-- Identify transactions beyond expected window
SELECT 
    transaction_id,
    transaction_date
FROM 
...
Implementing validation rules ensures data meets quality standards:

```sql
-- Create a data quality validation table
...
VALUES 
    ('email_not_null', 'customers', 'email', 'completeness', 'Email must not be null'),
    ('valid_email_format', 'customers', 'email', 'accuracy', 'Email must match valid format pattern'),
    ('positive_price', 'products', 'price', 'validity', 'Price must be greater than zero'),
    ('valid_category', 'products', 'category', 'validity', 'Category must be in allowed list');

-- Run validation checks
-- Example: Check completeness rule
SELECT 
...
A governance framework establishes policies and procedures for data management:

```sql
-- Create a data catalog for tracking data assets
...
-- Populate catalog
INSERT INTO data_catalog (schema_name, table_name, table_description, data_owner, data_steward, source_system, is_pii)
VALUES 
    ('public', 'customers', 'Customer master data', 'Sales Team', 'Data Team', 'CRM', TRUE),
    ('public', 'products', 'Product catalog', 'Product Team', 'Operations', 'ERP', FALSE),
    ('public', 'sales_transactions', 'Sales transaction records', 'Finance Team', 'Data Team', 'POS', FALSE);

-- Query catalog for PII tables
SELECT 
    table_name,
    data_owner,
    is_pii,
...
-- Record lineage
INSERT INTO data_lineage (source_table, source_column, target_table, target_column, transformation_rule)
VALUES 
    ('staging.customers', 'cust_name', 'dim_customer', 'customer_name', 'TRIM, UPPER'),
    ('staging.customers', 'cust_email', 'dim_customer', 'email', 'LOWER, NULLIF empty'),
    ('staging.transactions', 'trans_total', 'fact_sales', 'sales_amount', 'ROUND(2)');
```
...
Creating queries that can power data quality monitoring dashboards:
...
WITH quality_metrics AS (
    SELECT 
        'customers' as table_name,
        ROUND(COUNT(customer_id) * 100.0 / NULLIF(SUM(1) OVER (), 0), 2) as completeness_score
    FROM customers
...
SELECT 
    ...
    completeness_score,
    CASE 
        WHEN completeness_score >= 95 THEN 'Good'
...
SELECT 
    ...
    rule_name,
    violations,
    LAG(violations) OVER (PARTITION BY rule_name ORDER BY DATE(checked_at)) as previous_violations,
    violations - LAG(violations) OVER (PARTITION BY rule_name ORDER BY DATE(checked_at)) as change
FROM 
    quality_check_results
WHERE 
    checked_at >= CURRENT_DATE - INTERVAL '30 days'
ORDER BY 
    rule_name, check_date;
```

#### Handling Data Quality Issues

```sql
-- Deduplication strategy: Identify and remove duplicates
...
Protecting sensitive data while enabling analytics requires a comprehensive security approach. This section covers authentication, authorization, row-level security, and data masking for analytical databases.
...
GRANT SELECT, INSERT, UPDATE ON customers TO analyst;
GRANT SELECT, INSERT, UPDATE, DELETE ON staging_tables TO data_engineer;

-- Grant schema-level permissions
GRANT USAGE ON SCHEMA analytics TO analyst;
GRANT ALL ON SCHEMA analytics TO data_engineer;

-- Grant role membership
GRANT report_viewer TO analyst;
GRANT analyst TO data_engineer;
GRANT admin TO data_engineer;

-- Revoke permissions when needed
REVOKE DELETE ON sales_transactions FROM analyst;
```
...
```sql
-- Create view that excludes sensitive columns for general users
...
GRANT SELECT ON customer_basic TO report_viewer;

-- Sensitive columns only accessible to authorized roles
CREATE VIEW customer_sensitive AS
SELECT 
    customer_id,
    customer_name,
    email,
...
GRANT SELECT ON customer_sensitive TO admin;
```

#### Row-Level Security (RLS)

Implement RLS to ensure users only see their own data:

```sql
-- Enable RLS on a table
ALTER TABLE sales_transactions ENABLE ROW LEVEL SECURITY;

-- Create a policy for users to see only their data
CREATE POLICY own_transactions_policy ON sales_transactions
    FOR SELECT
    USING (customer_id = CURRENT_USER_VARIABLE());
...
-- Policy for managers to see their team's data
CREATE POLICY team_leader_policy ON employee_data
    FOR SELECT
    USING (
        employee_id IN (
            SELECT employee_id 
...
FROM 
    pg_policies
WHERE 
    tablename IN ('sales_transactions', 'customers', 'employee_data');
```

#### Data Masking

Protect sensitive data through dynamic or static masking:

```sql
-- Dynamic data masking (PostgreSQL example)
...
GRANT SELECT ON customers_masked TO report_viewer;
```

```sql
-- Static masking with separate tables for different sensitivity levels

-- Create masked table for general access
CREATE TABLE customers_masked_static AS
SELECT 
    customer_id,
    customer_name,
    CASE 
        WHEN email IS NOT NULL THEN 
            LEFT(email, 2) || '***@' || SUBSTRING(email FROM POSITION('@' IN email) + 1)
        ELSE NULL
    END as email,
    city,
    state,
...
-- Create fully sensitive table for authorized users
CREATE TABLE customers_full AS
SELECT * FROM customers;

-- Set up permissions
GRANT SELECT ON customers_masked_static TO report_viewer;
GRANT SELECT ON customers_full TO admin;

-- Schedule regular masking refresh
-- Could be implemented via cron or database job
REFRESH MATERIALIZED VIEW customers_masked_static;  -- if using materialized view
```

#### PII Protection Compliance

```sql
-- Identify PII columns across the database
SELECT 
    table_schema,
    table_name,
    column_name,
...
WHERE 
    ...
    OR column_name ILIKE '%social_security%'
...
CREATE TABLE pii_columns (
    ...
    compliance_requirement VARCHAR(50)  -- 'GDPR', 'HIPAA', 'PCI-DSS', etc.
);

-- Populate with known PII columns
INSERT INTO pii_columns (table_schema, table_name, column_name, pii_type, compliance_requirement)
VALUES 
    ('public', 'customers', 'email', 'email', 'GDPR'),
    ('public', 'customers', 'phone', 'phone', 'GDPR'),
    ('public', 'customers', 'ssn', 'ssn', 'HIPAA'),
    ('public', 'customers', 'dob', 'date_of_birth', 'HIPAA'),
    ('public', 'orders', 'credit_card', 'credit_card', 'PCI-DSS');
...
CREATE TABLE pii_access_log (
    ...
    access_type VARCHAR(10)  -- 'SELECT', 'UPDATE', etc.
);
...
BEGIN
    IF TG_OP = 'SELECT' THEN
        INSERT INTO pii_access_log (user_name, table_name, column_name, access_type)
        VALUES (
            CURRENT_USER,
            TG_TABLE_NAME,
            COALESCE(NULLIF(NEW.email, OLD.email), OLD.email),
            'SELECT'
        );
    END IF;
    RETURN NULL;
...
CREATE TRIGGER pii_access_audit
...
-- Encryption function
CREATE OR REPLACE FUNCTION encrypt_column(value TEXT, key_bytes BYTEA)
RETURNS BYTEA AS $$
BEGIN
...
-- Decryption function
CREATE OR REPLACE FUNCTION decrypt_column(value BYTEA, key_bytes BYTEA)
RETURNS TEXT AS $$
BEGIN
    RETURN pgp_sym_decrypt(value, key_bytes);
END;
$$ LANGUAGE plpgsql;

-- Encrypt existing data (with proper key management)
UPDATE customers
SET email_encrypted = encrypt_column(email, 'your-encryption-key'::BYTEA)
WHERE email IS NOT NULL;

-- Drop plaintext column after encryption
ALTER TABLE customers DROP COLUMN email;

-- Access decrypted data through function (with proper authorization)
CREATE FUNCTION get_decrypted_email(key_bytes BYTEA)
RETURNS TABLE (customer_id INTEGER, email TEXT) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        c.customer_id,
        decrypt_column(c.email_encrypted, key_bytes)::TEXT as email
    FROM 
        customers c
    WHERE 
        has_table_privilege('admin', 'customers', 'SELECT');
END;
$$ LANGUAGE plpgsql;

-- Transparent Data Encryption (TDE) - database level
-- Note: This is database-specific, shown for PostgreSQL
ALTER DATABASE analytics_db SET session_preload_libraries = 'pg_tde';
```
...
BEGIN
    INSERT INTO security_audit_log 
        (user_name, event_type, table_name, column_name, old_value, new_value, success, error_message)
    VALUES 
        (CURRENT_USER, p_event_type, p_table_name, p_column_name, p_old_value, p_new_value, p_success, p_error_message);
END;
$$ LANGUAGE plpgsql;
...
GROUP BY 
    DATE(event_timestamp), user_name, table_name
ORDER BY 
    date DESC, access_count DESC;
...
FROM 
    security_audit_log
WHERE 
    event_timestamp >= CURRENT_DATE - INTERVAL '24 hours'
    AND event_type = 'SELECT'
GROUP BY 
    user_name
HAVING 
    COUNT(DISTINCT table_name) > 20;  -- Flag users accessing many tables
```
...
GRANT ALL ON DATABASE analytics_db TO analyst;  -- Too permissive

-- Good practice: Granular permissions
GRANT SELECT ON SCHEMA analytics TO analyst;
GRANT SELECT ON analytics.sales_view TO analyst;
GRANT EXECUTE ON FUNCTION calculate_ltv TO analyst;
...
WHERE 
    grantee IN ('analyst', 'report_viewer')
ORDER BY 
...
### 7. Data Warehouse and Lake Architecture

Modern data architectures have evolved beyond traditional data warehouses to address diverse analytical needs, various data types, and processing requirements. Understanding these architectural patterns is essential for designing effective analytics platforms.

#### Medallion Architecture

The medallion architecture is a data design pattern that organizes data into progressive layers, named after the medals awarded in competitions (bronze, silver, gold). Each layer represents a stage of data transformation and refinement.

```sql
-- BRONZE LAYER (Raw Data)
-- Contains ingested data in its most raw form, typically append-only
-- Preserves original structure and format from source systems
...
-- Validated, cleaned, deduplicated, and enriched data
...
-- Business-ready tables optimized for analytics and reporting
-- Fully denormalized star schema or aggregated views
...
```sql
-- Query pattern: Tracking data lineage through layers
SELECT 
    'Bronze' as layer,
    COUNT(*) as record_count,
...
SELECT 
    'Gold' as layer,
    COUNT(*) as record_count,
    MIN(date_value) as earliest_record,
...
A data lake stores raw data at any scale, supporting diverse data types and analytics workloads. Unlike data warehouses, data lakes can handle unstructured and semi-structured data.

```sql
-- Data Lake Zone Architecture

-- Landing Zone: Raw ingestion (equivalent to Bronze)
CREATE TABLE lake_landing (
    file_id BIGSERIAL PRIMARY KEY,
    file_path VARCHAR(500),
    file_format VARCHAR(20),  -- JSON, CSV, PARQUET, AVRO
    source_system VARCHAR(100),
    ingestion_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Raw Zone: Cataloged but unprocessed data
...
-- Curated Zone: Cleaned and transformed data (similar to Silver)
CREATE TABLE lake_curated (
...
|--------|-----------|----------------|
| Data Types | Raw, unstructured, semi-structured | Structured, typed |
| Schema | Schema-on-read | Schema-on-write |
| Users | Data scientists, engineers | Business analysts |
| Use Cases | ML, exploration, ad-hoc queries | Reporting, BI, dashboards |
| Cost | Lower storage cost, higher compute | Higher storage, optimized compute |

#### Data Lakehouse

The lakehouse architecture combines the best of data lakes and data warehouses, providing:

```sql
-- Lakehouse: Unified storage layer with warehouse-like capabilities

-- Raw layer (object storage)
CREATE TABLE lakehouse_raw (
    ingestion_id BIGSERIAL PRIMARY KEY,
...
-- Silver layer: Cleaned and validated with ACID transactions
CREATE TABLE lakehouse_silver (
...
-- Gold layer: Aggregated for BI/analytics
CREATE TABLE lakehouse_gold_revenue (
    date_key INTEGER,
    product_category VARCHAR(50),
    region VARCHAR(50),
...
-- Time travel: Query historical versions (lakehouse capability)
SELECT * FROM lakehouse_silver
...
Data mesh is an organizational and architectural approach that treats data as a product, with domain-oriented decentralized data ownership and architecture.

```sql
-- Domain-oriented data products

-- Each domain owns their data product
CREATE TABLE marketing_data_products.campaign_metrics (
...
CREATE TABLE sales_data_products.order_summary (
...
CREATE TABLE customer_data_products.customer_profile (
    customer_id INTEGER PRIMARY KEY,
    customer_name VARCHAR(100),
    email VARCHAR(100),
    segment VARCHAR(20),
...
-- Federated governance: Cross-domain joins with standardized keys
SELECT 
    m.campaign_name,
    c.customer_segment,
    COUNT(DISTINCT o.order_id) as order_count,
    SUM(o.total_amount) as revenue
FROM marketing_data_products.campaign_metrics m
JOIN sales_data_products.order_summary o ON m.campaign_id = o.campaign_id
JOIN customer_data_products.customer_profile c ON o.customer_id = c.customer_id
WHERE m.start_date >= DATE_TRUNC('month', CURRENT_DATE)
GROUP BY m.campaign_name, c.customer_segment;
```

#### Kimball vs Inmon Methodologies

Two primary approaches to building data warehouses:

```sql
-- KIMBALL: Bottom-up, dimensional modeling
-- Focus: Subject-oriented, denormalized star schemas
-- Start with business processes

-- Fact table for sales (grain: individual transaction)
CREATE TABLE kimball_fact_sales (
    sale_key BIGSERIAL PRIMARY KEY,
    date_key INTEGER REFERENCES dim_date(date_key),
    customer_key INTEGER REFERENCES dim_customer(customer_key),
    product_key INTEGER REFERENCES dim_product(product_key),
...
-- INMON: Top-down, normalized 3NF
-- Focus: Enterprise-wide normalized data warehouse
-- Starts with enterprise data model

-- Normalized enterprise tables
CREATE TABLE inmon_order (
    order_id INTEGER PRIMARY KEY,
    order_date DATE,
    customer_id INTEGER REFERENCES inmon_customer(customer_id),
    status VARCHAR(20)
);

CREATE TABLE inmon_order_line (
    line_id INTEGER PRIMARY KEY,
    order_id INTEGER REFERENCES inmon_order(order_id),
    product_id INTEGER REFERENCES inmon_product(product_id),
    quantity INTEGER,
    unit_price NUMERIC(12,2)
);

CREATE TABLE inmon_customer (
    customer_id INTEGER PRIMARY KEY,
    customer_name VARCHAR(100),
...
FROM inmon_order o
JOIN inmon_order_line ol ON o.order_id = ol.order_id
JOIN inmon_product p ON ol.product_id = p.product_id
JOIN inmon_customer c ON o.customer_id = c.customer_id
GROUP BY o.order_date, p.product_category, c.customer_region;
```
...
-- Small to medium analytics needs: Consider cloud data warehouse
-- Examples: Snowflake, BigQuery, Redshift, Synapse

-- Large-scale ML and AI workloads: Consider data lakehouse
-- Examples: Databricks, Snowflake with UNISTORE, Redshift Spectrum

-- Multiple business domains with varying needs: Consider data mesh
-- Federated governance with domain ownership

-- Rapid ingestion of diverse data sources: Consider lake-first
-- Lambda or Kappa architecture for streaming + batch

-- Example: Multi-layer architecture decision
...
INSERT INTO architecture_decisions VALUES
('Real-time dashboards', 'Streaming + Warehouse', 
 'Low latency requirements', 'Kafka + Snowflake + dbt'),
 
('ML model training', 'Data Lakehouse', 
 'Large-scale feature engineering', 'Databricks + Delta Lake'),
 
('Regulatory reporting', 'Kimball Data Mart', 
 'Consistent business definitions', 'dbt + Redshift'),
 
('Multi-tenant SaaS', 'Data Mesh', 
 'Domain autonomy required', 'Snowflake + Terraform');
```
